...
| 2026-06-15 | Driver modal: removed redundant "TELEMETRY FILE" explorer panel + `selectDriver()`; fixed career-points chart via `PLOT_WHEN_VISIBLE` (width-gated newPlot) | dashboard/index.html | Modal bar chart rendered at width≈0 → bars crushed on left axis; explorer duplicated the modal |
...
| 2026-08-03 | Dashboard refocus merged to `main` and deployed: outrun visual system (Orbitron + Share Tech Mono, neon-as-chrome / livery-as-data), new Race Strategy section, typed scroll reveals, telemetry-traced 3D circuits | dashboard/, DESIGN.md, tokens | The page read as one more dark analytics dashboard; PRODUCT.md names that as an anti-reference and the register is brand, where safe is invisible |
| 2026-08-03 | Daily data sync (`.github/workflows/sync-data.yml`) + seed regression guard (`scripts/check-seed-regression.mjs`) | CI, scripts/, test/ | The Worker cron refreshed KV but nothing refreshed the COMMITTED seed, so the offline-first render aged from generation. The guard exists because reseeding on 2026-08-03 silently dropped openf1 11 rounds → 9 and 677 stints → 508 when OpenF1 served partial data, and every test still passed |
